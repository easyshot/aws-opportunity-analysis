...
function updateSqlGenerationDebugEnhanced(debugInfo) {
  ...
  if (accuracyIndicator) {
    accuracyIndicator.textContent = sqlInfo.enhancedAccuracy ? 'ENABLED' : 'DISABLED';
    accuracyIndicator.className = `accuracy-indicator ${sqlInfo.enhancedAccuracy ? 'enabled' : 'disabled'}`;
  }
...
function extractSqlGenerationInfoEnhanced(debugInfo) {
  ...
  const promptId = debugInfo?.promptId || 
