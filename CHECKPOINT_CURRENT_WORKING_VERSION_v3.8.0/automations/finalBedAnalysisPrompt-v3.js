...
function processConverseApiResponse(response) {
  ...
  // ENHANCED DEBUG: Log the complete Bedrock response structure to diagnose section extraction failure
  console.log("=== BEDROCK RESPONSE SECTION EXTRACTION DEBUG START ===");
  console.log("Response length:", messageContentText.length);
  console.log("First 2000 characters of response:");
  console.log(messageContentText.substring(0, 2000));
  console.log("Last 1000 characters of response:");
  console.log(messageContentText.substring(Math.max(0, messageContentText.length - 1000)));
  ...
  console.log("Searching for section markers...");
  ...
  // Show line-by-line analysis of first 50 lines to understand structure
  const lines = messageContentText.split('\n').slice(0, 50);
  console.log("First 50 lines of response (line-by-line analysis):");
  lines.forEach((line, index) => {
    if (line.includes('===') || line.includes('##') || line.includes('**')) {
      console.log(`  Line ${index + 1}: "${line}" <-- POTENTIAL SECTION MARKER`);
    } else if (line.trim().length > 0) {
      console.log(`  Line ${index + 1}: "${line.substring(0, 100)}${line.length > 100 ? '...' : ''}"`);
    }
  });
  
  console.log("=== BEDROCK RESPONSE SECTION EXTRACTION DEBUG END ===");
  ...
  try {
    ...
    function extractSection(sectionName) {
      ...
      console.log(`\n🔍 EXTRACTION DEBUG: Trying to extract section "${sectionName}"`);
      
      for (let i = 0; i < patterns.length; i++) {
        const regex = patterns[i];
        console.log(`   Pattern ${i + 1}: ${regex.toString()}`);
        const match = messageContentText.match(regex);
        if (match) {
          console.log(`   ✅ Pattern ${i + 1} MATCHED! Found ${match.length} groups`);
          if (match[1]) {
            ...
            if (extracted.length > 0) {
              ...
              console.log(`   ⚠️  Extracted content was empty after trimming`);
            }
          } else {
            console.log(`   ⚠️  Pattern matched but group 1 was empty`);
          }
        } else {
    ...
    // Debug: Show extracted section lengths and content previews
    console.log("🔍 PROCESS_RESULTS (Analysis): SECTION EXTRACTION RESULTS:");
    console.log(`   📋 methodologyText: ${methodologyText.length} chars - "${methodologyText.substring(0, 100)}${methodologyText.length > 100 ? '...' : ''}"`);
    console.log(`   📊 findingsText: ${findingsText.length} chars - "${findingsText.substring(0, 100)}${findingsText.length > 100 ? '...' : ''}"`);
    console.log(`   💡 rationaleText: ${rationaleText.length} chars - "${rationaleText.substring(0, 100)}${rationaleText.length > 100 ? '...' : ''}"`);
    console.log(`   ⚠️  riskFactorsText: ${riskFactorsText.length} chars - "${riskFactorsText.substring(0, 100)}${riskFactorsText.length > 100 ? '...' : ''}"`);
    console.log(`   📈 similarProjectsText: ${similarProjectsText.length} chars - "${similarProjectsText.substring(0, 100)}${similarProjectsText.length > 100 ? '...' : ''}"`);
    console.log(`   📖 fullAnalysisText: ${fullAnalysisText.length} chars`);
    
    // Summary of section extraction success
    const extractedSections = [
      { name: 'methodology', length: methodologyText.length },
      { name: 'findings', length: findingsText.length },
      { name: 'rationale', length: rationaleText.length },
      { name: 'riskFactors', length: riskFactorsText.length },
      { name: 'similarProjects', length: similarProjectsText.length }
    ];
    const successfulExtractions = extractedSections.filter(s => s.length > 0).length;
    console.log(`📊 Section extraction summary: ${successfulExtractions}/${extractedSections.length} sections successfully extracted`);
    
    if (successfulExtractions === 0) {
      console.log("🚨 CRITICAL: NO SECTIONS WERE EXTRACTED! This will cause fallback text to appear in frontend.");
      console.log("🔧 DEBUGGING NEEDED: Check Bedrock response format and section extraction patterns.");
    }
    ...
    // Extract meaningful content from full analysis when sections are empty - NO FALLBACK TEXT
    const extractFromFullAnalysis = (sectionType) => {
      ...
      if (sectionType === "findings" && findingsText.length === 0) {
        ...
        return findingsMatch ? findingsMatch[1].trim() : "";
      }
      ...
      if (
        ...
        return similarMatch ? similarMatch[1].trim() : "";
      }
      return "";
    };
    ...
    return {
      ...
      // Add individual sections for frontend compatibility - NO FALLBACK TEXT, show extraction errors
      methodology: methodologyText || extractFromFullAnalysis("methodology") || "❌ ERROR: Methodology section could not be extracted from AI response",
      findings: findingsText || extractFromFullAnalysis("findings") || "❌ ERROR: Findings section could not be extracted from AI response", 
      riskFactors: riskFactorsText || extractFromFullAnalysis("riskFactors") || "❌ ERROR: Risk factors section could not be extracted from AI response",
      similarProjects: similarProjectsText || extractFromFullAnalysis("similarProjects") || "❌ ERROR: Similar projects section could not be extracted from AI response",
      rationale: rationaleText || extractFromFullAnalysis("rationale") || "❌ ERROR: Rationale section could not be extracted from AI response",
