...
exports.execute = async (params) => {
  ...
  try {
    ...
    console.log(
      `Selected prompt ${promptId} for query generation. Reason: ${selectionReason}`
    );
    ...
    // Step 6: Validate and fix SQL query if needed
    const validatedResults = validateAndFixSQLQuery(processedResults);
    ...
    return {
      status: "success",
      processResults: validatedResults,
      promptMetadata: {
    ...
    return {
      ...
      processResults: JSON.stringify({
        sql_query: "SELECT 'Error generating query'",
      }),
    };
  }
};
 ...
 */
function validateAndFixSQLQuery(processedResults) {
  try {
    // Extract SQL query from the processed results
    let sqlQuery = "";
    ...
    if (typeof processedResults === "string") {
      ...
      return processedResults; // Return as-is if no SQL found
    }

    console.log("🔍 Validating SQL query for syntax issues...");

    // Fix nested lower() function calls
    // Pattern: lower(lower(column) LIKE 'pattern')
    const nestedLowerPattern =
      /lower\s*\(\s*lower\s*\(\s*([^)]+)\s*\)\s*LIKE\s*'([^']+)'/gi;
    sqlQuery = sqlQuery.replace(
      nestedLowerPattern,
      (match, column, pattern) => {
        console.log(
          `🔧 Fixed nested lower() function: ${match} -> lower(${column}) LIKE '${pattern}'`
        );
        return `lower(${column}) LIKE '${pattern}'`;
      }
    );

    // Fix other common SQL issues
    // Fix double lower() in CASE statements
    const caseLowerPattern =
      /CASE\s+WHEN\s+lower\s*\(\s*lower\s*\(\s*([^)]+)\s*\)\s*\)/gi;
    sqlQuery = sqlQuery.replace(caseLowerPattern, (match, column) => {
      console.log(
        `🔧 Fixed CASE lower() function: ${match} -> CASE WHEN lower(${column})`
      );
      return `CASE WHEN lower(${column})`;
    });

    // Fix boolean expressions in lower() function
    const booleanLowerPattern = /lower\s*\(\s*([^)]+)\s+OR\s+([^)]+)\s*\)/gi;
    sqlQuery = sqlQuery.replace(booleanLowerPattern, (match, expr1, expr2) => {
      console.log(
        `🔧 Fixed boolean lower() function: ${match} -> (lower(${expr1}) OR lower(${expr2}))`
      );
      return `(lower(${expr1}) OR lower(${expr2}))`;
    });
    ...
    return processedResults; // Return original if validation fails
  }
}
...
function preparePayload(params, promptData) {
  try {
    ...
    throw new Error(
      "Failed to prepare Bedrock payload for SQL query: " + error.message
    );
  }
}
...
async function invokeBedrockConverse(payload) {
  ...
  throw new Error(
    `Failed to invoke Bedrock after ${maxRetries} attempts: ${lastError.message}`
  );
}
...
function processConverseApiResponse(response) {
  ...
  if (
    ...
    console.error(
      "Invalid Bedrock response structure:",
      JSON.stringify(response, null, 2)
    );
    throw new Error(
      "Invalid or incomplete Bedrock Converse API response structure for SQL query generation."
    );
  }
...
function attemptFallbackParsing(messageContent) {
  try {
    ...
    if (sqlMatch) {
      ...
      console.log(
        "Fallback parsing extracted SQL:",
...
async function recordPerformanceMetrics(
  ...
  try {
    ...
    await promptManager.recordPromptPerformance(
      "query-generation",
      promptId,
      metrics
    );
    ...
    console.log(
      `Recorded performance metrics for prompt ${promptId}:`,
      metrics
    );
  } catch (error) {
